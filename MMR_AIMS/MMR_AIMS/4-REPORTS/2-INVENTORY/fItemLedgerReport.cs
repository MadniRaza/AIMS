...
namespace MMR_AIMS
{
    public partial class fItemLedgerReport : Form
    {
        #region Data Fields
        int ID = 0;
        #endregion
        public fItemLedgerReport()
        {
            InitializeComponent();
        }
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dtItem = ((DataSet)modelItem.Get(ID)).Tables[0].Copy();
                DataSet ds = (DataSet)modelItem.GetItemLedger(ID, dtpFrom.Value, dtpTo.Value);
                ...
                dtItemLedger.TableName = "ItemLedger";
                ...
                frm.ReportPath = Path.Combine(AppData.IsLive ? AppDomain.CurrentDomain.BaseDirectory : Directory.GetParent(Directory.GetCurrentDirectory()).Parent.FullName,"4-Reports","2-Inventory","rptItemLedger.rdlc");
                frm.HeaderText = "Item Ledger Report";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    ID = Convert.ToInt32(result["ItemId"]);
                    txtName.Text = result["ItemName"].ToString();
                }
            }
            catch (Exception ex)
            {
